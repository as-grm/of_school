...
def replacePatches(fn):
    ...
    while True:
        if 'inlet' in line:
            ...
            new_lines = replaceItem(iter_lines, 'type', '\ttype            patch;')
            replaced.extend(new_lines)
            new_lines = replaceItem(iter_lines, 'physicalType', '\tinGroups        patch;')
            replaced.extend(new_lines)
        elif 'outlet' in line:
            replaced.append(line.rstrip())
            new_lines = replaceItem(iter_lines, 'type', '\ttype            patch;')
            ...
            replaced.append(line.rstrip())
            new_lines = replaceItem(iter_lines, 'type', '\ttype            symmetryPlane;')
            replaced.extend(new_lines)
            new_lines = replaceItem(iter_lines, 'physicalType', '\tinGroups        1(symmetryPlane);')
            replaced.extend(new_lines)
        elif 'bottom' in line:
            replaced.append(line.rstrip())
            new_lines = replaceItem(iter_lines, 'type', '\ttype            symmetryPlane;')
