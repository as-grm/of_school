...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,'q:',['quality='])
    except getopt.GetoptError:
        print('cylinder_block_mesh_generator.py -q <coarse, fine, calc>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h': # help block
            print ('cylinder_block_mesh_generator.py -q <coarse, fine, calc>')
    ...
    L = 0.1       # length scale in flow [m]; length of surface
    yPlus = 1   # desidered y+
    ...
    print()
    print('Mesh quality: ', mesh_quality)

    # coarse mesh
    if mesh_quality == 'coarse':
        # number of divions cels
        nd_bl = 5     # boundary layer direction
        nd_nose = 5   # from tip to mid
        nd_tail = 5   # from tail to mid
        nd_mid = 5    # middle foil section
        nd_front = 10  # front part, form nose to inlet
        nd_back = 30  # back part , from tail to outlet
        nd_tb = 10     # top-bottom part, from mid to TB
        #scaling factors
        scale_bl = '1.2' # scale factor in BL
        scale_nose = 1   # scale factor for nose part
        scale_tail = 1   # scale fctor for tail part
        scale_mid = 1    # scale factor for mid part
        scale_front = 4  # scale factor of front part
        scale_back = 4   # scale factor of back part
        scale_tb = 4     # scale factor of top-bottom direction

    # fine mesh
    elif mesh_quality == 'fine':
        # number of divions cels
        nd_bl = 30     # boundary layer direction
        nd_nose = 40   # from tip to mid
        nd_tail = 40   # from tail to mid
        nd_mid = 20    # middle foil section
        nd_front = 40  # front part, form nose to inlet
        nd_back = 150  # back part , from tail to outlet
        nd_tb = 40     # top-bottom part, from mid to TB
        # scaling factors
        scale_bl = '3'   # scale factor in BL
        scale_nose = 3   # scale factor for nose part
    ...
    # fine mesh
        ...
        print('ERROR: Unknown mesh quality', mesh_quality)
